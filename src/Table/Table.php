 ...
 */
abstract class Table extends Registry implements TableInterface, DispatcherAwareInterface
{
	...
	public static function getInstance($type, $prefix = 'Table', $config = array())
	{
		...
		if (!class_exists($tableClass))
		{
			...
			{
				if ($tryThis = Path::find($paths[$pathIndex++], $tableClass . '.php'))
				{
...
//echo $class_name;
	...
	protected function _getAssetParentId(Table $table = null, $id = null)
	{
		...
		$assets = self::getInstance('Asset', 'Table', array('dbo' => $this->getDbo()));
	...
	{
		if ($input instanceof AccessRules)
		{
			$this->_rules = $input;
		}
		else
		{
			$this->_rules = new AccessRules($input);
		}
	}
