 ...
 * @since  11.1
 */
class TableMenu extends TableNested
{
	/**
	 * Constructor
	 *
	...
	public function check()
	{
		...
		if ($this->parent_id == 1 && in_array($this->alias, Folder::folders(JPATH_ROOT)))
		{
	...
	public function store($updateNulls = false)
	{
		...
		$table = Table::getInstance('Menu', 'Table', array('dbo' => $this->getDbo()));

		$originalAlias = trim($this->alias);
		$this->alias   = !$originalAlias ? $this->title : $originalAlias;
		$this->alias   = OutputFilter::stringURLSafe(trim($this->alias), $this->language);
		...
		else
		{
			...
			if ($error)
			{
				$menuTypeTable = JTable::getInstance('MenuType', 'Table', array('dbo' => $this->getDbo()));
